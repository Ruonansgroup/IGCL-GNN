...
class XC(torch.nn.Module):
    ...
    def forward(self, x,edge_index, batch,eval_random=True):
        ...
        xc_edge_att, xc_node_att = edge_att[:, 0], node_att[:, 0].view(-1, 1)
        xc = xc_node_att * x
        xc = F.elu(self.conv(self.bn(xc), edge_index, xc_edge_att))
        xc = self.dropout(xc)
        xc = self.global_pool(xc, batch)
        return xc, xc_edge_att, xc_node_att, edge_att, node_att
...
class XO(torch.nn.Module):
    ...
    def forward(self,x,edge_index, batch, eval_random=True):
        edge_att, node_att = self.att_cov(x, edge_index)
        xo_edge_att, xo_node_att = edge_att[:, 1], node_att[:, 1].view(-1, 1)
        xo = xo_node_att * x
        xo = F.elu(self.conv(self.bn(xo), edge_index, xo_edge_att))
        xo = self.dropout(xo)
        xo = self.global_pool(xo, batch)
        return xo, xo_edge_att, xo_node_att


# Acausation_Classifier
...
# Causal_Interventions_Classifier
...
class CausalGCN(torch.nn.Module):
    ...
    def __init__(self, num_features,
        ...
        self.attcov = Att_cov(hidden,hidden)
        self.Big = BIG(num_features,args,bns_conv,convs)
        self.xc = XC(args,self.attcov,gfn,edge_norm)
        self.xo = XO(args,self.attcov,gfn,edge_norm)
    ...
    def forward(self, data, eval_random=True):
        ...
        xc_, xc_edge_att, xc_node_att, edge_att_, node_att_ = self.xc(x, edge_index, batch, eval_random)
        xo_, xo_edge_att, xo_node_att = self.xo(x, edge_index, batch, eval_random)
        ...
        return xc_logis, xo_logis, xco_logis, xc_pres, edge_att_, node_att_
...
class CausalGAT(torch.nn.Module):
    ...
    def forward(self, data, eval_random=True):
        ...
        xc_, xc_edge_att, xc_node_att, edge_att_, node_att_ = self.xc(x, edge_index, batch, eval_random)
        xo_, xo_edge_att, xo_node_att = self.xo(x, edge_index, batch, eval_random)
        # edge_att_ = torch.cat((xc_edge_att.view(-1, 1), xo_edge_att.view(-1, 1)), dim=1)
        # node_att_ = torch.cat((xc_node_att, xo_node_att), dim=1)
