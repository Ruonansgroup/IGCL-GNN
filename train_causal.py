...
def train_causal_real(dataset=None, model_func=None, args=None,test_data_set = None):
    ...
    for fold, (train_idx, test_idx, val_idx) in enumerate(zip(*k_fold(dataset, args.folds, args.epoch_select))):
        ...
        print("syd: Causal fold:[{}] | Dataset:[{}] Model:[{}] | Best Test:[{:.2f}] at epoch [{}] | Test_o:[{:.2f}] Test_c:[{:.2f}] (RG:{:.2f})"
                .format(fold,
                        ...
                        best_test_acc_c * 100,
                        random_guess * 100))
    savefile = 'saved_test/loss/' + args.dataset + str(args.seed) + '/' + args.model + args.train_model
    if args.double_co_use_mgda:
        savefile = savefile + '2mgda'
    if args.mgda_with_double_co:
        savefile = savefile + '2co'
    ...
    savefile = 'saved_test/ACC/' + args.dataset + '/' + args.model + args.train_model
    if args.double_co_use_mgda:
        savefile = savefile + '2mgda'
    if args.mgda_with_double_co:
        savefile = savefile + '2co'
    ...
    # todo
    test_acc_max, test_acc_max_indices = torch.max(test_acc, dim=1)  # 找到每行的最大准确率和对应的epoch索引
    savefile_path = f'saved_test/ACC/{args.dataset}{args.seed}/{args.model}/'
    filename = f'{args.model}{args.train_model}_max_test_acc.txt'
    savefile = os.path.join(savefile_path, filename)
    os.makedirs(savefile_path, exist_ok=True)  # 确保保存的路径存在
    with open(savefile, 'w') as f:
        for acc, index in zip(test_acc_max, test_acc_max_indices):
            f.write(f"{acc.item()} {index.item()}\n")  # 将Tensor的值转换为Python的数值并写入
    ...
    test_acc_max_mean = test_acc_max.mean().item()  # todo
    test_acc_max_std = test_acc_max.std().item()  # todo
    ...
    print("=" * 150)
    print('sydall Final: Causal | Dataset:[{}] Model:[{}] seed:[{}]| Max Acc: {:.2f}±{:.2f} | best_epoch = {:.2f}|Test Acc: {:.2f}±{:.2f} | OTest: {:.2f}±{:.2f}, CTest: {:.2f}±{:.2f} (RG:{:.2f}) | [Settings] co:{},c:{},o:{},harf:{},dim:{},fc:{} | epoch use time :{:.6f}'
         .format(args.dataset,
                 args.model,
                 args.seed,
                 test_acc_max_mean * 100,
                 test_acc_max_std * 100,
                 best_epochs_mean,
                 test_acc_mean * 100, 
                 test_acc_std * 100,
                 test_acc_o_mean * 100, 
...
def eval_acc_causal(model, loader, device, args):
    ...
    for data in loader:
        data = data.to(device)
        with torch.no_grad():
            c_logs, o_logs, co_logs, _, _, _ = model(data, eval_random=eval_random)
